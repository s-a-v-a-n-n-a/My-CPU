...
const stack_elem POISON = -4000000000000;
...
stack_code is_pointer_valid (Stack *that_stack)
{
    ...
    if (*(unsigned int *)that_stack != 0x5E7CA6E || *(unsigned int *)that_stack->stack != 0xDEADB14D || that_stack->stack->buffer[0] != POISON ||
                                                    *(unsigned int *)that_stack->stack_copy != 0xDEADB14D || that_stack->stack_copy->buffer[0] != POISON)
    {
...
stack_code stack_verifier (Stack **that_stack)
{
    ...
    if ((*that_stack)->stack->length >= (*that_stack)->stack->capacity)
    {
        printf("LET ME KNOW\n");
    ...
    long int flag_eq              = fabs(((*that_stack)->stack->buffer[(*that_stack)->stack->length] -
                                     (*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length])) < 0;// < EPSILON;
    ...
    if ((*that_stack)->canary_first == 0x5E7CA6E && (*that_stack)->canary_last == 0x0FFCA6E)
    {
        ...
        if (flag_eq && (((*that_stack)->stack->buffer[(*that_stack)->stack->length]) != POISON ||
                         ((*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length] != POISON)))
        {
            return STACK_DELETED;
        }
        ...
        else if (!flag_eq && (*that_stack)->stack->buffer[(*that_stack)->stack->length] != POISON ||
             (!flag_hash_stack && flag_hash_copy && !flag_hash_stack_buf && flag_hash_copy_buf))
        {
        ...
        //          (!flag_hash_stack && flag_hash_copy && !flag_hash_stack_buf && flag_hash_copy_buf))
        else if (!flag_eq && (*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length] != POISON ||
             (!flag_hash_stack && flag_hash_copy && !flag_hash_stack_buf && flag_hash_copy_buf))
        {
        ...
        else if (!(flag_hash_stack && flag_hash_copy && flag_hash_stack_buf) && flag_hash_copy_buf)
        {
            ...
            while((*that_stack)->stack_copy->buffer[i] != POISON)
            {
                new_len++;
                i++;
            }
        ...
        else if (!(flag_hash_stack && flag_hash_copy && flag_hash_copy_buf) && flag_hash_stack_buf)
        {
            ...
            while((*that_stack)->stack->buffer[i] != POISON)
            {
                new_len++;
                i++;
            }
...
static stack_code stack_construct(Structure **that_stack, size_t stack_size)
{
    ...
    (*that_stack)->buffer[0]   = POISON;// NAN;
    (*that_stack)->buffer[1]   = POISON;// NAN;
    (*that_stack)->length      = 1;
    (*that_stack)->hash_buffer = hashing_buffer(*that_stack);
    (*that_stack)->hash_stack  = hashing_stack (*that_stack);
...
stack_code stack_push(Stack **that_stack, const stack_elem value)
{
    ...
    (*that_stack)->stack->buffer[(*that_stack)->stack->length]   = POISON;//NAN;
    ...
    (*that_stack)->stack_copy->buffer[(*that_stack)->stack_copy->length]   = POISON;//NAN;
